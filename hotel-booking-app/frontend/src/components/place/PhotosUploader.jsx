...
import { uploadByLink, uploadPhotos } from "../../api/uploadApi.js";
...
export default function PhotosUploader({addedPhotos,onChange}) {
  ...
  async function addPhotoByLink(ev) {
    ev.preventDefault();
    const {data:filename} = await uploadByLink(photoLink);
    onChange(prev => {
  ...
  function uploadPhoto(ev) {
    ...
    uploadPhotos(data).then(response => {
    ...
    <>
      ...
      <div className="mt-2 grid gap-3 grid-cols-3 lg:grid-cols-8 md:grid-cols-5">
            ...
            <div className="h-32 flex relative" key={link}>
                <img className="rounded-2xl w-full object-cover" src={'https://hotel-booking-backend-tbfn.onrender.com/uploads/'+link} alt=""/>
